...
extern int get_batt_therm_touch(void);
extern int get_batt_present_touch(void);
...
static void mit_battery_thermal(struct mms_data *ts, char caller)
{
	...
	ret = (short)get_batt_therm_touch();
	
	if (ret >= 2000 || ret == (short)0xFFF)
		goto DATA_ERROR;

		TOUCH_INFO_MSG("Thermal value %d %s\n", ret, (caller ? "[IC]" : ""));
		if(ret == -300){
			batt_present = get_batt_present_touch();

			if(batt_present == 0){
				TOUCH_INFO_MSG("No battery , ret = %d, change ret = 300 \n", ret);
				ret = 300;
			}else if(batt_present == 1){
				TOUCH_INFO_MSG("battery Present , ret = %d \n", ret);
			}else if(batt_present == -1){
				TOUCH_INFO_MSG("battery Present Read Error , ret = %d \n", ret);
				goto DATA_ERROR;
			}
		}
...
err_event_type:
	TOUCH_ERR_MSG("Unkown, err event type 0x%x\n", event_type);
	return -EIO;
mms_error_event:
	TOUCH_ERR_MSG("Unkown, mms event type 0x%x\n", event_type);
	return -ENXIO;
}
