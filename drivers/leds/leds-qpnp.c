...
#if defined(CONFIG_MACH_MSM8926_JAGNM_ATT) || defined(CONFIG_MACH_MSM8926_JAGC_SPR) || defined(CONFIG_MACH_MSM8926_JAGNM_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGNM_KDDI_JP) || defined(CONFIG_MACH_MSM8226_JAG3GDS_GLOBAL_COM) || defined(CONFIG_MACH_MSM8226_JAG3GSS_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGN_KR) || defined(CONFIG_MACH_MSM8926_B2LN_KR) || defined(CONFIG_MACH_MSM8926_JAGDSNM_CN) || defined(CONFIG_MACH_MSM8926_AKA_KR)
...
#if defined(CONFIG_LGE_JAGDSNM_CABC)
	rc = qpnp_led_masked_write(led, WLED1_LED1_CABC_EN(led->base),
			WLED_CABC_EN_MASK,WLED_CABC_EN_ON);
	printk("%s:cabc write",__func__);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
				"WLED cabc_en reg write failed(%d)\n",rc);
		printk("cabc_failed rc = %d\n",rc);
		return rc;
	}
#endif
	/* dump wled registers */
			...
			struct pwm_config_data *pwm_cfg, int type, int offMS, int onMS, int blinking)
{
	...
	|| defined(CONFIG_MACH_MSM8926_JAGNM_TLS) || defined(CONFIG_MACH_MSM8926_JAGNM_VTR) || defined(CONFIG_MACH_MSM8926_JAGNM_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGNM_KDDI_JP) || defined(CONFIG_MACH_MSM8226_JAG3GDS_GLOBAL_COM) \
        || defined(CONFIG_MACH_MSM8226_JAG3GSS_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGDSNM_CN) || defined(CONFIG_MACH_MSM8926_JAGN_KR) || defined(CONFIG_MACH_MSM8926_B2LN_KR)
			...
			int blue_flag, int period)
{
	...
	for (i = 0; i < 63; i++){
		...
		if(window_color == WINDOW_COLOR_BK)
			input_patterns[i] = patterns[i] / 2;
		else if(window_color == WINDOW_COLOR_WH)
			input_patterns[i] = patterns[i] * 45 / 100;
		else
