...
#ifdef CONFIG_QPNP_DISABLE_HARD_RESET
#define QPNP_PON_S3_CTL(base)           (base + 0x74)
#endif
...
static int __devinit qpnp_pon_config_init(struct qpnp_pon *pon)
{
	...
	while ((pp = of_get_next_child(pon->spmi->dev.of_node, pp))) {
		...
		case PON_KPDPWR:
          ...
          #ifdef CONFIG_QPNP_DISABLE_HARD_RESET
            pr_info("[QPNP-POWER-ON] Disable power long key reset!!\n");
            rc = qpnp_pon_masked_write(pon, cfg->s2_cntl2_addr,
                                QPNP_PON_S2_CNTL_EN, 0);
            rc = qpnp_pon_masked_write(pon,
                        QPNP_PON_S3_CTL(pon->base), 0x3, 0x1);
          #endif
			break;
		case PON_RESIN:
			cfg->state_irq = spmi_get_irq_byname(pon->spmi,
