...
/* LGE_CHANGE_S, yt.jeon@lge.com, Flash driver B/U 2013-10-04*/
	...
	if (fctrl->flash_i2c_client) {
		...
		pr_err("%s: strobe current : 862.5mA\n", __func__);
		rc =flash_write_reg(fctrl->flash_i2c_client,	0x06, 0x18);
#elif defined(CONFIG_MACH_MSM8X10_L70P)
		/* Configuration of current, torch : 56.25mA, strobe : 478.125mA */
		pr_err("%s: strobe current : 956.25mA\n", __func__);
		rc =flash_write_reg(fctrl->flash_i2c_client,	0x06, 0x19);
#else
		/* Configuration of current, torch : 56.25mA, strobe : 384.375mA */
		pr_err("%s: strobe current : 768.75mA\n", __func__);
