...
periph_interrupt(struct spmi_pmic_arb_dev *pmic_arb, u8 apid, bool show)
{
	...
	if (!is_apid_valid(pmic_arb, apid)) {
		dev_err(pmic_arb->dev,"periph_interrupt(apid:0x%x sid:0x%x pid:0x%x) unknown peripheral\n",
       apid, sid, pid);
		/* return IRQ_NONE; */
	}
